...
import { ENV_INFO, API_CONFIG, GOOGLE_CONFIG } from "../config/environment.js";
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    // Only try to load Google API if client ID is properly configured
    if (GOOGLE_CONFIG.clientId && GOOGLE_CONFIG.clientId !== 'your-google-client-id') {
    loadGoogleAPI()
      .then(() => {
      ...
      .catch((err) => {
          console.warn('⚠️ Google OAuth failed to load (this is OK, normal login still works):', err);
          setIsGoogleLoaded(false);
          // Don't set error for Google OAuth failures - normal login should still work
      });
    } else {
      console.warn('⚠️ Google Client ID not configured, skipping Google OAuth');
      setIsGoogleLoaded(false);
    }
  }, []);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="bg-white/90 backdrop-blur-lg p-8 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100/50 animate-fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-sm">
            {isLogin ? 'Sign in to continue to ResearchNet' : 'Join the research community'}
          </p>
        </div>
        
        {/* Google Sign-In Section - Only show if Google is properly configured */}
        {GOOGLE_CONFIG.clientId && GOOGLE_CONFIG.clientId !== 'your-google-client-id' && (
        <div className="mb-6">
          ...
          {isGoogleLoaded ? (
            ...
            <div className="mb-4">
              <button
                  onClick={() => setError("Google OAuth not available. Please use email/password login.")}
                  className="w-full bg-gray-400 text-white py-3 rounded-lg cursor-not-allowed flex items-center justify-center font-medium"
              >
          ...
          <form onSubmit={isLogin ? handleLogin : handleRegister}>
            ...
            <div className="mb-4">
              ...
              <input
                  ...
                  className={`w-full px-4 py-3 pr-12 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white/80 backdrop-blur-sm transition-all duration-200 ${
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3.5 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.02] disabled:hover:scale-100"
            >
              {loading ? (
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-blue-600">
              {API_CONFIG.fallbackMode ? 'Fallback Mode' : 'Real Backend Mode'}
            </span>
          </div>
          ...
          <p className="text-xs text-orange-600 mt-1">
            {API_CONFIG.fallbackMode 
              ? '⚠️ Using fallback mode - backend not required' 
